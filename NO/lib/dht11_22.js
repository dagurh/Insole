...
class DHT11_22 {
  ...
  /*
     ...
     */
  measure (callback) {
    if (this._callback) { throw new Error('already measuring'); }
  ...
  start (callback, waitMS) {
    ...
    this._againMS = waitMS === undefined ? 1000 : waitMS;
    this.measure(callback);
  }
  ...
  _fall (stamp) {
    if (this._callback && this._riseStamp !== undefined) {
      const dur = stamp - this._riseStamp;
      delete this._riseStamp;
      // Â Â     console.log(this._n, dur);
      ...
      if (dur >= 0.015 && dur <= 0.047) {
        if (this._n <= 16) { this._hum = this._hum << 1; } else if (this._n <= 32) { this._temp = this._temp << 1; } else { this._check = this._check << 1; }
      } else if (dur <= 0.1) {
        if (this._n <= 16) { this._hum = (this._hum << 1) | 1; } else if (this._n <= 32) { this._temp = (this._temp << 1) | 1; } else { this._check = (this._check << 1) | 1; }
      } else {
      ...
      if (this._n == 40) {
        ...
        if ((this._check & 0xFF) != (((this._hum >> 8) + this._hum + (this._temp >> 8) + this._temp) & 0xFF)) { cb(new Error('wrong checksum')); } else {
          if (this._temp & 0x8000) { this._temp = -(this._temp & 0x7FFF); }
          if (this._hum & 0x8000) { this._hum = -(this._hum & 0x7FFF); }
